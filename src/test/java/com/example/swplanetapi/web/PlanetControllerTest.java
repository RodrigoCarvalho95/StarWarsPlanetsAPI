...
public class PlanetControllerTest {
	...
	public void listPlanets_ReturnFilteredPlanets() throws Exception {
		...
		when(planetService.list(ALDERAAN.getTerrain(), null)).thenReturn(List.of(ALDERAAN));
		...
		mockMvc.perform(get("/planets?terrain=" + ALDERAAN.getTerrain()))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$", hasSize(1)))
			.andExpect(jsonPath("$[0]").value(ALDERAAN));
	}
